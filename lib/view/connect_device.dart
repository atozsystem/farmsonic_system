...
class _ConnectDeviceScreenState extends State<ConnectDeviceScreen> {
  ...
  Future<void> _connectToWifi(String ssid) async {
    try {
      ...
      if (ssid == "FARM_AP") {
        ...
        if (isConnected) {
          print("연결 성공: $ssid");
          _showSnackbarMessage("연결 성공!");
          _launchInAppBrowser('http://192.168.0.1');
        } else {
          print("연결 실패");
  ...
  Widget build(BuildContext context) {
    return WillPopScope(  // 뒤로 가기 버튼 비활성화
      onWillPop: () async {
        // false를 반환하면 뒤로 가기 버튼을 막음
        return false;
      },
      child: Scaffold(
        key: _scaffoldKey,
